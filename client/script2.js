  ...
  data.forEach((user) => {
    userDisplay.innerHTML += `
      ...
      <tr>
          ...
          <td><input type="button" onClick="editPhone(${user.id}, '${user.puhelin}', '${user.nimi}')"value="Muokkaa"</button>
      </tr>
      </tbody>
...
function editPhone(id, phonenum, nimi) {
  const newPhone = prompt("Anna uusi puhelinnumero:", phonenum);
  if (!newPhone) return;

  fetch(`http://localhost:3000/items/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ id, nimi, puhelin: newPhone }) // koko objekti
  })
  .then(() => loadPage()) // päivitetään taulukko
